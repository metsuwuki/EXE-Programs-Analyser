...
fn run(config: Config) {
    ...
    let bytes = match preflight_and_load(&config.exe_path, target_kind, &mut findings) {
        Ok(data) => data,
        Err(_) => {
    ...
    let runtime = if target_kind == TargetKind::Executable {
        run_runtime_checks(&config, &mut findings)
    } else {
...
fn preflight_and_load(path: &Path, target_kind: TargetKind, findings: &mut Vec<Finding>) -> Result<Vec<u8>, ()> {
    if !path.exists() {
        findings.push(finding(
            Severity::Fail,
            "FILE_NOT_FOUND",
...
fn run_runtime_checks(config: &Config, findings: &mut Vec<Finding>) -> Vec<RunResult> {
    ...
    for scenario in scenarios {
            ...
            match child.try_wait() {
                ...
                Err(e) => {
                    findings.push(finding(
                        ...
                        "runtime",
                        20,
                        format!("Scenario '{}' wait failed: {}", scenario.name, e),
                    ));
                    break;
                }
            }
        ...
        if timed_out {
            findings.push(finding(
                ...
                "runtime",
                25,
                format!("Scenario '{}' exceeded timeout {} sec", scenario.name, scenario.timeout_secs),
            ));
            ...
            if is_windows_crash_exit(code) {
                findings.push(finding(
                    ...
                    "runtime",
                    30,
                    format!("Scenario '{}' crashed with NTSTATUS-like code {:#X}", scenario.name, code as u32),
                ));
            } else if code == 0 {
                findings.push(finding(
                    Severity::Pass,
        ...
        if let Some(last) = results.last() {
            println!(
                ...
                last.scenario, last.exit_code, last.timed_out, last.duration_ms
            );
        }
    }
...
fn write_report_files(
    ...
    if let Err(e) = fs::create_dir_all(&config.out_dir) {
        return Err(format!("Failed to create output dir '{}': {}", config.out_dir.display(), e));
    }
    ...
    fs::write(&full_log, full).map_err(|e| format!("Write full log failed: {}", e))?;
    fs::write(&issues_log, issues).map_err(|e| format!("Write issues log failed: {}", e))?;
    ...
    fs::write(&json_log, json).map_err(|e| format!("Write JSON report failed: {}", e))?;
