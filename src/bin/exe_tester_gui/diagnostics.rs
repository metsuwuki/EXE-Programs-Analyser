...
pub(crate) fn parse_debug_source_location(line: &str) -> Option<DebugSourceLocation> {
    ...
    if let Some(loc) = parse_gdb_location(trimmed) {
        return Some(loc);
    }
...
pub(crate) fn infer_exit_code_from_output(stdout: &str, stderr: &str) -> Option<i32> {
    let combined = format!("{}\n{}", stdout, stderr);
    ...
    if let Some(code) = parse_exit_code_marker(&lower, "exited with code ") {
        return Some(code);
    }
...
fn parse_exit_code_marker(text: &str, marker: &str) -> Option<i32> {
    ...
    if let Some(hex) = tail.strip_prefix("0x") {
        ...
        let raw = u32::from_str_radix(&hex_digits, 16).ok()?;
...
fn parse_gdb_location(line: &str) -> Option<DebugSourceLocation> {
    let marker = " at ";
    let marker_pos = line.find(marker)?;
    let candidate = line[marker_pos + marker.len()..].trim();
    if candidate.is_empty() {
        return None;
    }

    let (path_part, line_part) = candidate.rsplit_once(':')?;
